...
export const getBookingById = asyncHandler(async (req, res, next) => {
  ...
  const booking = await Booking.findById(bookingId)
    .populate({
      path: "propertyId",
      select:
        "name policies images address city state contactNumber email cancellationPolicy childrenCharge",
    })
    .populate({ path: "userId", select: "name email phone profilePicture" })
    .populate({
      path: "rooms.roomId",
      select: "name typeOfRoom images amenities description capacity",
    })
